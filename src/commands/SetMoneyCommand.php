...
final class SetMoneyCommand extends BaseCommand {
    ...
    protected function prepare(): void {
        PermissionManager::getInstance()->addPermission(new Permission(Economy::getInstance()->getConfig()->getNested("economy.commands.setmoney-permission") ?? "setmoney.use"));
                ...
                if (isset($args["player"], $args["amount"])) {
                    ...
                    if (Economy::getInstance()->getProvider()->exist($player)) {
                        if (is_int($args["amount"])) {
                            if ($args["amount"] > 0) {
                                Economy::getInstance()->getProvider()->set($player, $args["amount"]);
