...
final class AddMoneyCommand extends BaseCommand {
    ...
    protected function prepare(): void {
        PermissionManager::getInstance()->addPermission(new Permission(Main::getInstance()->getConfig()->getNested("economy.commands.addmoney.permission") ?? "addmoney.use"));
    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void {
        if ($sender instanceof Player) {
            if ($sender->hasPermission($this->config->getNested("economy.commands.addmoney.permission") ?? "addmoney.use")) {
                if (isset($args["player"], $args["amount"])) {
                    ...
                    if (Main::getInstance()->getProvider()->exist($player)) {
                        if (is_int($args["amount"])) {
                            if ($args["amount"] > 0) {
                                Main::getInstance()->getProvider()->add($player, $args["amount"]);
